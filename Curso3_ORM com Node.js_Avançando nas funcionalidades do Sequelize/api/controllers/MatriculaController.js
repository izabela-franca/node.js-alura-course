...
class MatriculaController {
  ...
  static async pegaMatriculasPorTurma(req, res) {   //Requisito 6: listar matriculas por turma
    const { turmaId } = req.params
    try {
      const todasAsMatriculas = await matriculasServices
        .encontraEContaRegistros(
          { turma_id: Number(turmaId), status: 'confirmado' },
          { limit: 20, order: [['estudante_id', 'DESC']] }) //Limitanto paginação, 20 arquivos por pag
  ...
  static async pegaTurmasLotadas(req, res) {  //Requisito 7: limitar número de matrículas por turma
      ...
      const turmasLotadas = await matriculasServices
        .encontraEContaRegistros({ status: 'confirmado' },
          { 
            attributes: ['turma_id'],
            group: ['turma_id'],
            having: Sequelize.literal(`count(turma_id) >= ${lotacaoTurma}`) //utilizando métodos sequelize
          })
